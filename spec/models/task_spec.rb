...
RSpec.describe 'タスクモデル機能', type: :model do
  describe 'バリデーションのテスト' do
    ...
    context 'タスクのタイトルと詳細に内容が記載されている場合' do
      it 'バリデーションが通る' do
        task = Task.new(title: '成功テスト', content: '成功テスト', priority: '高', status: '完了', deadline: '2023-5-16')
  ...
  describe '検索機能' do
    ...
    let!(:task) { FactoryBot.create(:task, title: 'task', status: '着手') }
    ...
    context 'scopeメソッドでタイトルのあいまい検索とステータス検索をした場合' do
      it "検索キーワードをタイトルに含み、かつステータスに完全一致するタスク絞り込まれる" do
        expect(Task.get_by_title('task').get_by_status('着手')).to include(task)
        expect(Task.get_by_title('sample').get_by_status('完了')).to include(second_task)
        expect(Task.get_by_title('task').get_by_status('着手').count).to eq 1
      end
    end
  end
end
